...
const transformApiUserToUser = (apiUser: ApiUser): User => {
  return {
    ...
    email: apiUser.email,
    phone: apiUser.phone,
    profilePicture: undefined, // ApiUser doesn't have this field
    role: 'farmer' as UserRole, // Default role, as ApiUser doesn't specify
    location: apiUser.location ? {
      district: apiUser.location.split(',')[0]?.trim() || '',
      country: apiUser.location.split(',')[1]?.trim() || '',
    } : undefined,
...
export const updateUserProfile = createAsyncThunk(
  ...
  async (userData: Partial<User>, { getState, rejectWithValue }) => {
    try {
      ...
      // Transform User data to ApiUser format
      const apiUserData: Partial<Omit<ApiUser, 'id' | 'createdAt' | 'updatedAt'>> = {
        ...
        ...(userData.location && { 
          location: typeof userData.location === 'string' 
            ? userData.location 
            : `${userData.location.district}, ${userData.location.country}`
        })
      };
...
export const { clearError, updateUser, setRefreshing, resetUserStats } = authSlice.actions;
